...
async function scanAdd(fileProps: FileProps) {
...
async function scanRemove(fileProps: FileProps) {
  const occurence = await findDb('scan', { _id: fileProps.id }, FileProps);
  if (occurence.length !== 1) {
    console.error(`Invalid number of scan found [${occurence.length}] for`, fileProps, occurence);
    return;
  }
  ...
  if (updatedDoc[0] !== occurence[0].dbFiles.length) {
    console.error(
      `Incorrect dbFilesRef updated (${updatedDoc[0]}), expected (${occurence[0].dbFiles.length})`,
      occurence[0],
      updatedDoc[1]
    );
  }
...
export function scanProcessFile(fileProps: FileProps) {
  return async (dispatch: (action: Action) => void, getState) => {
    ...
    if (occurences.length === 0) {
      ...
      if (occurences.length > 1) {
        ...
        console.warn('We will only compare to the one with less differences in properties...');
        // throw Error(`Multiple occurences from hash ${newFileProps.hash}!!`);
      }
      newFileProps.setDbMatches(occurences);
      const compared: Array<string> = newFileProps.compareSameHash();
...
export function removeAllFiles(scanType: ConstScanType) {
  return async (dispatch: (action: Action) => void, getState) => {
    dispatch(startScanAction());
...
export function removeFile(file: FileProps) {
  return async (dispatch: (action: Action) => void, getState) => {
    dispatch(startScanAction());
    deleteFile(getState().foldersState.toScanPath, file.relpath);
    await scanRemove(file);
...
export function dbFilePropUpdated(dbFile: FilePropsDb) {
  return async (dispatch: (action: Action) => void) => {
    dispatch(startScanAction());
